...
class ProductController extends Controller
{
    ...
    public function detail_product($cate,$slug)
    {
        ...
        ])->where(['slug'=>$slug, 'status'=>1, 'cate_slug'=>$cate])->first(['id','name','images','type_cate','category','sku','discount','price','content','size','description','slug','preserve','cate_slug','type_slug','status']);
...
// return;
    ...
    public function getProductHome(Request $request) {
        ...
        $data['products'] = Product::where(['status'=>1, 'discountStatus'=>1 , 'category'=>$id])->get(['id','category','name','discount','price','images','slug','cate_slug','type_slug','description']);
        $html = view('layouts.product.hot-sale-product', $data)->render();
        return response()->json([
            'status' => 200,
            'html' => $html,
        ]);
